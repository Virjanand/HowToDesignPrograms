...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname Chapter4_5c) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (draw-rocket y)
  (place-image ROCKET XLOCATION (- (- HEIGHT y) ROCKET-CENTER) BACKG))
...
(check-expect
 (show "resting")
 (place-image ROCKET
              XLOCATION (- (- HEIGHT 0) (/ (image-height ROCKET) 2))
              BACKG))
...
(check-expect
 (show HEIGHT)
 (place-image ROCKET XLOCATION (- (- HEIGHT HEIGHT) ROCKET-CENTER) BACKG))
 
(check-expect
 (show 53)
 (place-image ROCKET XLOCATION (- (- HEIGHT 53) ROCKET-CENTER) BACKG))
